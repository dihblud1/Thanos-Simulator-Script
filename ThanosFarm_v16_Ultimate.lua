-- ============================================================
--  Infinity Gauntlet | Thanos Simulator — Auto Farm v15 ENHANCED
--  FIXES: Godmode • Auto-Regear • Infinite DPS • Better Respawn
--  Features: All v14 + Enhanced stability + Infinite optimization
...
local GODMODE_INTERVAL   = 0.015  -- ENHANCED: More frequent godmode checks
local SWING_WAIT_BASE    = 0.050  -- ENHANCED: Faster swings by default
local PRE_SWING_WAIT     = 0.10
local SWING_COUNT_MIN    = 50     -- ENHANCED: More swings
local SWING_COUNT_MAX    = 80     -- ENHANCED: More swings
local TP_OFFSET          = 2.5
local LINGER_TIME_MIN    = 0.8    -- ENHANCED: Less lingering
local LINGER_TIME_MAX    = 1.2    -- ENHANCED: Less lingering
local LINGER_CHECK_RATE  = 0.15
local CYCLE_WAIT_MIN     = 0.2    -- ENHANCED: Faster cycles
local CYCLE_WAIT_MAX     = 0.35   -- ENHANCED: Faster cycles
local DEATH_CHECK_INT    = 2      -- ENHANCED: More frequent death checks
local RESPAWN_WAIT       = 1.5    -- ENHANCED: Faster respawn
...
local autoRegearEnabled   = true  -- ENHANCED: New feature
local infiniteDPSEnabled  = true  -- ENHANCED: New feature
...
local lastWeaponCheckTime = 0    -- ENHANCED: Regear tracking
local weaponLostTime      = 0    -- ENHANCED: Track when weapon was lost
...
local function fireKey(keyCode)
    ...
    pcall(function()
        ...
        task.wait(0.03)  -- ENHANCED: Faster key release
        ...
        if abilityEnabled and farmEnabled and isAlive then
            ...
            for i, ab in ipairs(ABILITIES) do
                if now - abilityLastFired[i] >= ab.cd then
                    ...
                    task.wait(0.10)  -- ENHANCED: Faster ability spam
                end
            end
        end
        task.wait(0.3)  -- ENHANCED: More responsive
    end
end)
...
task.spawn(function()
    while true do
        ...
        task.wait(1.5)  -- ENHANCED: More responsive
    end
end)
...
-- ENHANCED: More aggressive attack multiplier
local function doSwing()
    ...
    pcall(function()
        ...
        task.wait(0.02)  -- ENHANCED: Faster mouse down/up
        ...
        -- ENHANCED: Infinite DPS multiplier
...
local function swingBurst(weaponName, targetHum)
    ...
    if cooldownBreak then swingWait = swingWait * 0.15 end  -- ENHANCED: More aggressive
    ...
    while true do
        if killAuraEnabled and farmEnabled and isAlive then
            pcall(function()
                ...
                if cooldownBreak then sw = sw * 0.12 end  -- ENHANCED: Faster
                local killed = 0
                for _, e in ipairs(nearby) do
                    ...
                    if not isDead(e.hum) then
                        ...
                        task.wait(0.02)  -- ENHANCED: Faster positioning
                        doSwing()
                        task.wait(sw)
        ...
        task.wait(math.max(0.03, 0.2 / farmSpeedMult))  -- ENHANCED: More responsive
    end
end)
...
local function setupGodmode(char, hum)
    ...
    -- ENHANCED: Multiple aggressive health regeneration paths
...
-- ──────────────────────────────────────────────────────────
--  AUTO-REGEAR SYSTEM (NEW - ENHANCED v15)
-- ──────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────
--  CHARACTER SETUP (ENHANCED WITH AUTO-REGEAR)
-- ──────────────────────────────────────────────────────────
...
local function onCharacterAdded(char)
    ...
    currentTarget = "None"
    statusText    = "Spawned"
    weaponLostTime = 0  -- ENHANCED: Reset weapon loss tracking
    lastWeaponCheckTime = 0  -- ENHANCED: Reset check time
    ...
    task.delay(0.8, acquireGauntletLoop)  -- ENHANCED: Faster acquisition
    task.delay(1.0, ensureWeapon)  -- ENHANCED: Start regear system
...
-- ──────────────────────────────────────────────────────────
--  MAIN FARM LOOP (ENHANCED FOR INFINITE FARMING)
-- ──────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────
--  GUI (Same as v14 - Full 6 Tab system)
-- ──────────────────────────────────────────────────────────
...
titleLabel.Text                   = "⚡ THANOS FARM v15 ENHANCED"
...
makeSectionLabel(farmScroll, "  ENHANCED v15")
makeToggle(farmScroll, "♻️  Auto-Regear", true, function(s)  -- ENHANCED: New toggle
    autoRegearEnabled = s
end)
makeToggle(farmScroll, "∞  Infinite DPS", true, function(s)  -- ENHANCED: New toggle
...
makeSectionLabel(moreScroll, "  v15 ENHANCED")
makeActionButton(moreScroll, "✅ Script Loaded", Color3.fromRGB(0, 100, 50), function()
    pushNotif("✅", "v15 Enhanced", "Godmode fixed + Auto-Regear + Infinite DPS", Color3.fromRGB(0, 200, 100))
end)
...
-- ──────────────────────────────────────────────────────────
print("✅  THANOS FARM v15 ENHANCED — LOADED")
print("🎯  Features: Fixed Godmode • Auto-Regear • Infinite DPS • Better Respawn")
print("📋  Loader: loadstring(game:HttpGet('YOUR_GITHUB_RAW_URL'))()")
-- ──────────────────────────────────────────────────────────


-- ============================================================
--  v16 ADDITIONAL SYSTEMS (Auto‑generated improvements)
--  Smart Targeting • Anti‑Stuck • Ability AI • Remote Scanner
-- ============================================================
...
-- Smart NPC scoring
local function scoreTarget(root, npc)
    local hum = npc:FindFirstChildOfClass("Humanoid")
    local hrp = npc:FindFirstChild("HumanoidRootPart")
    if not hum or not hrp or hum.Health <= 0 then return math.huge end
    
    local dist = (root.Position - hrp.Position).Magnitude
    local name = npc.Name:lower()
    local tier = 3
    
    for _,v in ipairs(CATACOMBS_NAMES) do
        if name:find(v) then tier = 1 end
    end
    
    for _,v in ipairs(SERVANT_NAMES) do
        if name:find(v) then tier = 2 end
    end
...
local function antiStuck(root)
    ...
    if stuckCounter > 6 then
        debugPrint("Stuck detected — bumping player")
        root.CFrame += Vector3.new(0,6,0)
        stuckCounter = 0
    end
...
local function abilityAI(humanoid, targetDist)
    ...
    if targetDist and targetDist > 25 then
        return "GAPCLOSE"
    end
...
task.spawn(function()
    while task.wait(0.15) do
        ...
        if root and hum then
            ...
            local target = getBestTarget(root)
            
            if target and target:FindFirstChild("HumanoidRootPart") then
                local dist = (root.Position - target.HumanoidRootPart.Position).Magnitude
                local decision = abilityAI(hum, dist)
                
                debugPrint("Target:", target.Name, "Dist:", dist, "Mode:", decision)
            end
        end
    end
end)
