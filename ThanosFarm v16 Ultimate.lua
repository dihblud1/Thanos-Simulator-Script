...
-- FIX: Weapons now match the official wiki exactly.
-- "Blitz", "Technoblade Sword", "Techno Blade" removed (not in wiki).
-- Added: Surtur's Sword variants, Upgraded variants, Hadron Enforcer, S.D.M.G, etc.
-- Order = highest priority first (Upgraded > Base).
...
-- FIX: Added Level 2, 3, 4 servant variants per wiki.
-- Wiki notes: Servant of Celestial [Rare] gives 9 kills — highest priority.
local SERVANT_NAMES = {
    "servant of the celestial [rare]",  -- prioritise rare boss (1.1M HP, 9 kills)
...
-- FIX: Added Z key (Beam) — wiki confirms this is the Upgraded Gauntlet's
-- most powerful move: "Using Beam (Z) and Focus (R) you can kill anything in seconds."
-- Removed T (Timestop) — not listed in the official wiki ability set.
-- Added X (Super Shockwave/Super Dash) per wiki.
local ABILITIES = {
...
local cachedAttackRemotes = {}   -- FIX: Single declaration (removed duplicate in v16 section)
...
-- FIX: Single regear loop flag to prevent multiple concurrent loops on respawn
...
-- ──────────────────────────────────────────────────────────
--  ABILITY AI — wiki-informed decision logic
--  Wiki: "If low HP use Focus [R] to heal 50k HP/second"
--        "Charge [Q] then Shield [C] to protect in dangerous scenarios"
-- ──────────────────────────────────────────────────────────

-- FIX: abilityAI now returns a mode that the ability loop actually uses.
...
-- FIX: Ability loop now integrates abilityAI mode.
-- Defensive mode: only fire Shield (C) and Focus/Heal (R).
-- Gap-close mode: prioritize Charge (Q).
-- Attack mode: fire all offensive abilities.
task.spawn(function()
    while true do
        if abilityEnabled and farmEnabled and isAlive then
            ...
            -- estimate distance to current target for AI
            if currentTarget ~= "None" and currentTarget ~= "Lingering..." then
                local cats, servs, other = require and {} or {}
                -- just use a broad check — full scan is too expensive every 0.3s
                for _, obj in ipairs(workspace:GetDescendants()) do
                    local h = obj:FindFirstChildOfClass("Humanoid")
                    local r = obj:FindFirstChild("HumanoidRootPart")
                    if h and r and h.Health > 0 and root then
                        local d = (root.Position - r.Position).Magnitude
                        if not targetDist or d < targetDist then
                            targetDist = d
                        end
                        break
                    end
            ...
            for i, ab in ipairs(ABILITIES) do
                if now - abilityLastFired[i] >= ab.cd then
                    ...
                    if aiMode == "DEFENSIVE" then
                        -- only fire defensive abilities
                        shouldFire = ab.defensive
                    elseif aiMode == "GAPCLOSE" then
                        -- prioritize Charge (Q) or offensive
                        shouldFire = (ab.key == Enum.KeyCode.Q) or not ab.defensive
                    else
                        -- ATTACK mode: fire all abilities in priority order
                        shouldFire = true
                    end
                    if shouldFire then
...
local function doSwing()
    ...
    pcall(function()
        ...
        local multiplier = (infiniteDPSEnabled and (rapidFireEnabled and rapidFireCount * 2 or rapidFireCount)) or 1
        fireAttackRemotes(multiplier)
    end)
end
...
-- FIX: Removed BillboardGui check — nearly all enemies in Thanos Simulator
-- use BillboardGuis as health bars / name tags. Blocking on this caused the
-- farm to find zero enemies. We now only block on ProximityPrompt (shop items)
-- and SurfaceGui (decorative shop signs).
local function isRealEnemy(model, hum)
    ...
    for _, v in ipairs(model:GetDescendants()) do
        ...
        -- SurfaceGui on shop NPCs is fine to block, but BillboardGui is NOT blocked
        -- (enemy health bars use BillboardGui)
    end
    return true
end
...
-- ──────────────────────────────────────────────────────────
--  SMART TARGET SCORING (v16 improvement)
--  Uses scanEnemies() results so it respects whitelist/blocked filters.
-- ──────────────────────────────────────────────────────────

local function getBestScoredTarget(root)
    if not root then return nil end
    local cats, servs, other = scanEnemies()
    local best, bestScore = nil, math.huge
    local function scoreEntry(entry, tierMult)
        local dist = (root.Position - entry.root.Position).Magnitude
        return dist + (tierMult * 40)
    end
    for _, e in ipairs(cats)  do
        local s = scoreEntry(e, 1)
        if s < bestScore then bestScore = s; best = e end
    end
    for _, e in ipairs(servs) do
        local s = scoreEntry(e, 2)
        if s < bestScore then bestScore = s; best = e end
    end
    for _, e in ipairs(other) do
        local s = scoreEntry(e, 3)
        if s < bestScore then bestScore = s; best = e end
    end
    return best
end
...
local function antiStuck(root)
    ...
    if stuckCounter > 6 then
        debugPrint("Stuck detected — bumping + random offset")
        -- FIX: teleport to a random offset rather than just going straight up
        root.CFrame = root.CFrame
            + Vector3.new(randomFloat(-8, 8), 5, randomFloat(-8, 8))
        stuckCounter = 0
    end
    lastPosition = root.Position
end
...
local function scanRemotes()
    -- FIX: Appends to the shared cachedAttackRemotes (no re-declaration)
    for _, v in ipairs(ReplicatedStorage:GetDescendants()) do
        if v:IsA("RemoteEvent") then
            ...
            if name:find("attack") or name:find("damage") or name:find("swing") then
                -- avoid duplicates
                local found = false
                for _, existing in ipairs(cachedAttackRemotes) do
...
local function swingBurst(weaponName, targetHum)
    ...
    local baseCount = math.random(SWING_COUNT_MIN, SWING_COUNT_MAX)
    local count = infiniteDPSEnabled and (baseCount * (1 + (cooldownBreak and 0.5 or 0))) or baseCount
    local swingWait = math.max(0.01, SWING_WAIT_BASE / farmSpeedMult)
    if cooldownBreak then swingWait = swingWait * 0.15 end
    for i = 1, count do
...
-- ──────────────────────────────────────────────────────────
--  AUTO-REGEAR SYSTEM
--  FIX: Uses regearLoopActive flag to prevent multiple concurrent loops
--  from stacking up across respawns.
-- ──────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────
--  ANTI-STUCK PERFORMANCE LOOP (v16)
-- ──────────────────────────────────────────────────────────
...
task.spawn(function()
    while task.wait(0.15) do
        ...
        if root and farmEnabled and isAlive then
            antiStuck(root)
            if DEBUG_MODE then
                local best = getBestScoredTarget(root)
                if best then
                    local dist = (root.Position - best.root.Position).Magnitude
                    debugPrint("BestTarget:", best.model.Name, "Dist:", dist)
                end
            end
        end
    end
...
titleLabel.Text                   = "⚡ THANOS FARM v16 ULTIMATE"   -- FIX: version label
...
local TAB_DEFS = {
    { icon = "🗡",  label = "FARM"   },
    { icon = "💥",  label = "COMBAT" },
    { icon = "✨",  label = "ABILITY" },  -- FIX: was misspelled "ABILTY"
    { icon = "📊",  label = "STATS"  },
    { icon = "📍",  label = "TP"     },
    { icon = "⚙️",  label = "MORE"   },
}
...
makeSectionLabel(farmScroll, "  v16 ULTIMATE")   -- FIX: was "ENHANCED v15"
makeToggle(farmScroll, "♻️  Auto-Regear", true, function(s)
    autoRegearEnabled = s
end)
makeToggle(farmScroll, "∞  Infinite DPS", true, function(s)
    infiniteDPSEnabled = s
end)
...
makeToggle(abilityScroll, "✨  Auto Abilities", false, function(s)
    ...
    if s then
        for i = 1, #ABILITIES do abilityLastFired[i] = 0 end
        -- FIX: Updated notification to list actual wiki keybinds including Z
...
makeSectionLabel(moreScroll, "  v16 ULTIMATE")   -- FIX: was "v15 ENHANCED"
makeActionButton(moreScroll, "✅ Script Loaded", Color3.fromRGB(0, 100, 50), function()
    pushNotif("✅", "v16 Ultimate", "Wiki Weapons • BillboardFix • Z-Beam • AI Abilities", Color3.fromRGB(0, 200, 100))
end)
...
print("🔧  FIXES: Wiki Weapons • BillboardGui NPC Bug • Z-Key Beam • Servant Levels")
print("🔧  FIXES: abilityAI Loop • Anti-Dupe Regear • Anti-Stuck Randomised")
